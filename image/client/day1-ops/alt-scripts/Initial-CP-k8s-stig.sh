...
# Force set net.ipv4.ip_forward=1
...
kubeadm init --control-plane-endpoint={{ lb_addr }}:6443 --upload-certs --apiserver-advertise-address={{ master_ip }} --pod-network-cidr=192.168.0.0/16 --cri-socket unix:///run/containerd/containerd.sock
...
# Install Pod network -- MUST BE COPIED OVER TO NODE
kubectl apply -f ~/calico-vxlan.yaml

# Create Certificate for Calico API Server 
openssl req -x509 -nodes -newkey rsa:4096 -keyout apiserver.key -out apiserver.crt -days 365 -subj "/" -addext "subjectAltName = DNS:calico-api.calico-apiserver.svc"
  
# Create Secret for Calico API Server 
kubectl create secret -n calico-apiserver generic calico-apiserver-certs --from-file=apiserver.key --from-file=apiserver.crt

# Install Calico API Server -- MUST BE COPIED OVER TO NODE
kubectl apply -f ~/calico-apiserver.yaml
    
# Create the command for joining the other master nodes
kubeadm token create --print-join-command --certificate-key $(kubeadm init phase upload-certs --upload-certs |  awk '{for (i=1;i<3;i++) {getline}; print $0}') > k8s_init_cp.txt
...
scp ./admissionControlConfig.yaml dd@HOST:/etc/kubernetes/config/admissionControlConfig.yaml
